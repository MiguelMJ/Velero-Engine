        ...
        while(w.pollEvent(e)){
            switch(e.type){
                ...
                case sf::Event::MouseButtonPressed:
                    rs.setVisible(&sprites1[i],true);
                    rs.setVisible(&sprites2[j],true);
                    i = (i+1)%3;
                    j = (j+1)%3;
                    v = !v;
                    rs.setVisible(&sprites1[i],false);
                    rs.setVisible(&sprites2[j],false);
                        rs.setVisible(&lamp1, v);
                    break;
                case sf::Event::KeyPressed:
                    if(e.key.code == sf::Keyboard::Space){
                        locked = !locked;
                        rs.setLocked(rs.getLayerId("ll"), locked);
                    }
                    break;
            }
        }
